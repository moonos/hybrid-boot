...
matches_uuid ()
{
	...
	for try_uuid_file in "${path}/.disk/hybrid-uuid"*
	do
...
get_backing_device ()
{
	case "${1}" in
		*.squashfs|*.ext2|*.ext3|*.ext4|*.jffs2)
			echo $(setup_loop "${1}" "loop" "/sys/block/loop*" '0' "${HYBRID_MEDIA_ENCRYPTION}" "${2}")
			;;
		...
		*)
			panic "Unrecognized hybrid filesystem: ${1}"
			;;
	esac
}
...
mount_images_in_directory ()
{
	...
	if is_hybrid_path "${directory}"
	then
		[ -n "${mac}" ] && adddirectory="${directory}/${HYBRID_MEDIA_PATH}/${mac}"
		setup_unionfs "${directory}/${HYBRID_MEDIA_PATH}" "${rootmnt}" "${adddirectory}"
	else
		panic "No supported filesystem images found at /${HYBRID_MEDIA_PATH}."
	fi
}
...
check_dev ()
{
	...
	then
		...
		then
			...
			then
				mkdir /hybrid/fromiso
				mount -t $fs_type "$ISO_DEVICE" /hybrid/fromiso
				ISO_NAME="$(echo $FROMISO | sed "s|$ISO_DEVICE||")"
				loopdevname=$(setup_loop "/hybrid/fromiso/${ISO_NAME}" "loop" "/sys/block/loop*" "" '')
				devname="${loopdevname}"
			else
	...
	if [ -n "${HYBRID_MEDIA_OFFSET}" ]
	then
		loopdevname=$(setup_loop "${devname}" "loop" "/sys/block/loop*" "${HYBRID_MEDIA_OFFSET}" '')
		devname="${loopdevname}"
	fi
	...
	then
		...
		[ -n "$devuid" ] && grep -qs "\<$devuid\>" /var/lib/hybrid/boot/devices-already-tried-to-mount && continue
		mount -t ${fstype} -o ro,noatime "${devname}" ${mountpoint} || continue
		[ -n "$devuid" ] && echo "$devuid" >> /var/lib/hybrid/boot/devices-already-tried-to-mount
		...
		then
			...
			then
				umount ${mountpoint}
				mkdir -p /hybrid/findiso
				mount -t ${fstype} -o ro,noatime "${devname}" /hybrid/findiso
				loopdevname=$(setup_loop "/hybrid/findiso/${FINDISO}" "loop" "/sys/block/loop*" 0 "")
				devname="${loopdevname}"
...
find_hybridfs ()
{
	...
	if [ -n "${HYBRID_MEDIA_TIMEOUT}" ]
	then
		if [ "${timeout}" -lt "${HYBRID_MEDIA_TIMEOUT}" ]
		then
			return 1
		fi
	fi
	...
	devices_to_scan="$(non_removable_dev 'sys')"
	...
	do
		...
		then
			...
			# hybrid-boot, so take it and run with it.
...
# Try to mount $device to the place expected by hybrid-boot. If $device
...
mount_persistence_media ()
{
	...
	# underneath /lib/hybrid/mounts/persistence, so we're done
	if [ -d "${device}" ]
	then
		echo "${device}"
		return 0
	...
	backing="/hybrid/persistence/$(basename ${device})"
	...
	then
		...
		# the place expected by hybrid-boot, so we're done.
		echo ${backing}
		return 0
	fi
}
...
find_persistence_media ()
{
	...
	#
	# The devices that are hosting the actual hybrid rootfs should not be
	...
	for d in /hybrid/rootfs/* /hybrid/findiso /hybrid/fromiso
	do
...
link_files ()
{
	...
	# hybrid-boot normally does (into $rootmnt)).
...
get_custom_mounts ()
{
	# Side-effect: leaves $devices with persistence.conf mounted in /hybrid/persistence
	...
	do
		...
		include_list="/etc/persistence.conf"

		if [ -n "${HYBRID_BOOT_DEBUG}" ] && [ -e "${include_list}" ]
		then
			cp ${include_list} /hybrid/persistence/${persistence_list}.${device_name}
		fi
		...
		do
			...
			if trim_path ${dir} | grep -q -e "^[^/]" -e "^/lib" -e "^/lib/hybrid\(/.*\)\?$" -e "^/\(.*/\)\?\.\.\?\(/.*\)\?$"
			then
				log_warning_msg "Skipping unsafe custom mount ${dir}: must be an absolute path containing neither the \".\" nor \"..\" special dirs, and cannot be \"/lib\", or \"/lib/hybrid\" or any of its sub-directories."
				continue
			fi
...
activate_custom_mounts ()
{
	...
	do
			...
			do
				...
				then
					...
					then
						...
						# FIXME: this should really be handled by hybrid-config since we don't know for sure which uid a certain user has until then
		...
		# we bootstrap it with $dest from the hybrid filesystem.
		...
		then
			link_files ${source} ${dest} "s|^/hybrid/|/lib/hybrid/mount/|"
		elif [ -n "${opt_link}" ] && [ -n "${PERSISTENCE_READONLY}" ]
		then
			mkdir -p ${rootmnt}/lib/hybrid/mount/persistence
			links_source=$(mktemp -d ${rootmnt}/lib/hybrid/mount/persistence/links-source-XXXXXX)
			...
			cow_dir="/hybrid/overlay/lib/hybrid/mount/persistence/$(basename ${links_source})"
			mkdir -p ${cow_dir}
