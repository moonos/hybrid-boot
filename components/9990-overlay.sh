...
setup_unionfs ()
{
	...
	then
		...
		do
			...
			maybe_break hybrid-realpremount
			log_begin_msg "Running /scripts/hybrid-realpremount"
			run_scripts /scripts/hybrid-realpremount
			log_end_msg
	...
	# tmpfs file systems
	touch /etc/fstab
	mkdir -p /hybrid/overlay
	mount -t tmpfs tmpfs /hybrid/overlay
	...
	then
		...
		then
			mount -t tmpfs -o rw,noatime,mode=755 tmpfs "/hybrid/overlay"
			root_backing="/hybrid/persistence/$(basename ${cowdevice})-root"
			mkdir -p ${root_backing}
		else
			root_backing="/hybrid/overlay"
		fi
	...
	then
		...
		then
			cow_dirs='/var/tmp /var/lock /var/run /var/log /var/spool /home /var/lib/hybrid'
		else
			cow_dirs=''
		fi
	else
	...
	for d in ${rootfslist}
	do
		hybrid_rootfs="/hybrid/rootfs/${d##*/}"
		hybrid_rootfs_list="${hybrid_rootfs_list} ${hybrid_rootfs}"
		mkdir -p "${hybrid_rootfs}"
		case "${d}" in
			*.dir)
				# do nothing # mount -o bind "${d}" "${hybrid_rootfs}"
				;;
			*)
				case "${UNIONTYPE}" in
					unionfs-fuse)
						mount -o bind "${d}" "${hybrid_rootfs}"
						;;

					*)
						mount -o move "${d}" "${hybrid_rootfs}"
						;;
				esac
				;;
		esac
	...
	then
		...
		[ -n "${HYBRID_BOOT_DEBUG}" ] && cp ${custom_mounts} "/lib/hybrid/mount/persistence"
        ...
        # otherways, initramfs-tools is unable to remove /hybrid
        # and fails to boot
        umount /hybrid/overlay > /dev/null 2>&1 || true
}
